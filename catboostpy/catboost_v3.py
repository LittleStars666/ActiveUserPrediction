...
def predict(clf2, test_set):
    ...
    print(active_users)
    str_time = str(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M"))
    submission_file = "result/submission_" + str_time + "catboost.csv"
...
def run():
    ...
    clf1 = GridSearchCV(CatBoostClassifier(),
                      param_grid={"iterations":[200,400,600],"learning_rate":[0.01,0.02,0.03]},
    ...
    paramsSpace = {
        ...
        'depth': hyperopt.hp.quniform("depth", 3, 6, 1),
