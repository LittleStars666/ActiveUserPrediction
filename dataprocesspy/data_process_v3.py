...
def processing(trainSpan=(1, 23), label=True):
    ...
    dtype_user_register = {"user_id": np.uint32, "register_day": np.uint8, "register_type": np.uint8,
                           "device_type": np.uint16,"register_day_rate":np.uint16,
    ...
    # app_launch_log = ["user_id","app_launch_day"]
    dtype_app_launch = {"user_id": np.uint32,
    ...
    # video_create_log = ["user_id", "video_create_day"]
    dtype_video_create = {"user_id":np.uint32,
    ...
    dtype_user_activity = {"user_id": np.uint32,
    ...
    df_launch_register_create_activity.drop_duplicates(subset=keep_feature, inplace=True)
    print("after drop the duplicates of the merged active dataset")
    ds5 = df_launch_register_create_activity.describe()
    print(ds5)
    ds5.to_csv("kuaishou_stats2.csv", mode='a')
    return df_launch_register_create_activity
