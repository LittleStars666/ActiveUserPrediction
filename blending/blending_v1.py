...
class EarlyStoppingSK(ClassifierMixin):
    def __init__(self, estimator, max_n_estimators, scorer,
                 n_min_iterations=100, scale=1.0001):
...
def stop_early_wrapper(classifier, train_x,train_y,val_x,val_y,n_min_iterations=100,scale=1.0001,eval_metric="auc"):
    ...
    early = EarlyStoppingSK(classifier,
                          ...
                          scorer=scorer,
                          n_min_iterations=n_min_iterations, scale=scale)
...
def rank_blending_predict(train_set, val_set, test_set, file, val_ratio=0.4, n_round=3):
    ...
    for i in range(n_round):
        ...
        temp_predict = blending_test(train_x,train_y,val_x,val_y,test_x,weights=(0.38,0.18,0.14,0.1,0.20))
