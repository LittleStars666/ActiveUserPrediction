...
def predict(clf2, test_set,param,kpca):
    ...
    # print(active_users)
    str_time = str(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    submission_file = "../result/622/submission_lgb_" +param+"_"+ str_time + ".csv"
...
def run(scheme_num=1,file_name="../data/data_v3/training_e"):
    ...
    # kpca = PCA(n_components=0.999,whiten=True)
    ...
    paramsSpace = {
        ...
        "scale_pos_weight": (0.98, 1.0, 'uniform'),
    }
    ...
    def tune_parameter(X, y, clf, params):
        ...
        gs = BayesSearchCV(
            ...
            scoring="f1", n_iter=120, optimizer_kwargs={"base_estimator": "GBRT"},
            verbose=1, n_jobs=-1, cv=3, refit=True, random_state=1234
        )
    ...
    clf2 = tune_parameter(train_set, train_label.values, model, paramsSpace)
    ...
    yhat = clf2.predict(val_set)
    print(classification_report(y_pred=yhat, y_true=val_label.values, digits=4))

    print("begin to make classification report for the training dataset")
    # yhat = clf2.predict(train_set.values)
    yhat = clf2.predict(train_set)
    print(classification_report(y_pred=yhat, y_true=train_label.values, digits=4))
    ...
    predict(clf2, test_set, param,kpca)
